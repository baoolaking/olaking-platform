-- Fix quantity constraint to allow wallet funding orders
-- This updates the quantity check to allow quantity = 1 for wallet funding orders

-- Step 1: Drop the existing quantity check constraint
ALTER TABLE orders DROP CONSTRAINT IF EXISTS orders_quantity_check;

-- Step 2: Add a new constraint that allows quantity = 1 for wallet funding, 100-500000 for services
